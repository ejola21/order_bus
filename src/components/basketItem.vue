...
<v-card
  ...
  >
    <div class="group">
        ...
        <div :style="{ 'background-image' : 'url(' + require('../assets/img/noimg.jpg') + ')' }" style="background-size: 100% 100%" class="item">
          <lazy-component>
            <img class="product_img" :src='item.product.img_thumb_url' @error='urlError'>
          </lazy-component>
        </div>
        <div class="item_desc">
          <img src="../assets/img/order_delete_btn.svg" @click="order_delete" class="delete"/>
          <div class="product_name_price">
            <div class="product_name">{{item.product.product_name}}</div>
            <div v-if="item.product.price_yn == 'N'" class="product_price">{{formatPrice(item.product.price, 'default') }}</div>
            <div v-else class="product_price">{{ item.product.sale_price > 0 ?  formatPrice(item.product.sale_price, 'default') : formatPrice(item.product.price, 'default') }}원</div>
    ...
    <div class="group2" v-if="options1.length > 0 ||  m_options.length > 0">
      <div class="options_div" v-if="options1.length > 0">
        ...
        <div class="options1_price" v-if="options1.length > 0">
          {{ options1[0].p_o_val > 0 ? '+' : '' }}{{ formatPrice(options1[0].p_o_val, 'default')}}원
        </div>
      </div>
      <div class="options_div" v-if="options2.length > 0">
        ...
        <div class="options2_price" v-if="options1.length > 0">
          {{ options2[0].p_o_val > 0 ? '+' : '' }}{{ formatPrice(options2[0].p_o_val, 'default')}}원
        </div>
      </div>
      <div class="options_div" v-for="add_option in m_options">
        ...
        <div class="options3_price">
          {{ add_option.add_option_price > 0 ? '+' : ''}}{{ formatPrice(add_option.add_option_price, 'default')}}원
        </div>
      </div>
    </div>
    ...
    <div v-if="item.product.price_yn == 'Y'" class="cnt_group">
      ...
      <div class="img-wrapper"><img src="../assets/img/remove_btn.svg" @click="cntMinus" width="30px" class="btn_img"/></div>
      <span>{{item.cnt}}</span>
      <div class="img-wrapper"><img src="../assets/img/add_btn.svg" @click="cntPlus" width="30px" class="btn_img"/></div>
    </div>
    <div class="item_total_price">
      <div class="one_item_price_title inline_block">총 주문 금액</div>
      <div class="one_item_total_price inline_block">{{formatPrice(item.price, 'default')}}원</div>
    </div>
 </v-card>
    </v-row>
  <!-- </mu-paper> -->
...
.cnt_item, .one_item_price_title, .one_item_total_price {
  font-size:1.1rem;
  width: 50%;
  font-weight: bold;
}
...
.options1, .options1_price, .options2, .options2_price, .options3, .options3_price {
  font-size: 1rem;
}
.options1, .options2, .options3 {
  float: left;
...
.options1_price, .options2_price, .options3_price {
  float: right;
  width:30%;
  text-align: right;
}
