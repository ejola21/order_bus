    ...
    <div class="address_info">
      ...
      <div class="address_div" v-show="Object.keys(addr).length == 0">
        <div class="text-center">
          <v-btn
            ...
            label="배달 주소 등록"
            class="add_address"
            labelClass="address_text_btn"
            @click="address_add_click"
          >
      ...
      <div class="address_div" v-show="Object.keys(addr).length > 0">
        <div class="add_address_have" v-if="store.delivery_name_yn === 'Y'">
          ...
          >
          <div class="address_change" @click="address_add_click">변경</div>
        </div>
        <div class="add_address_have" v-else>
          <span class="doroname"
            >{{ addr.address1 }}&nbsp;&nbsp;{{ addr.address_detail }}</span
          >
          <div class="address_change" @click="address_add_click">변경</div>
        </div>
      </div>
    ...
    <div class="pay_select">
      ...
      <div v-if="store.order_yn == 'Y'" class="pay_check_postpay">
        <div @click="postpay_click">
          ...
          <div
            :class="[
              { btn_box_order: store.pay_yn != 'Y' && store.order_yn == 'Y' },
              { btn_box: store.pay_yn == 'Y' },
            ]"
            v-show="select_radio == '후불'"
          >
            <div
              class="card_pay"
              :class="{ select_pay: delivery_pay_gb == 'C' }"
              @click="card_cash_click('C')"
            >
              <img
                class="icon_img"
                :src="delivery_pay_gb == 'C' ? img_s_card : img_n_card"
                width="23rem"
              />
              <span>&nbsp;카드</span>
            </div>
            <div
              class="appcard_pay"
              :class="{ select_pay: delivery_pay_gb == 'H' }"
              @click="card_cash_click('H')"
            >
              <img
                class="icon_img"
                :src="delivery_pay_gb == 'H' ? img_s_appcard : img_n_appcard"
                width="23rem"
              />
              <span>&nbsp;현금</span>
            </div>
...
export default {
  ...
  computed: {
    ...
    total_price() {
      ...
      else this.delivery_total_amt = _price;
      return _price;
    },
    u_name() {
  ...
  mounted() {
    ...
    if (this.user_id.length > 9) {
      ...
      (this.user_phone != "" && this.user_id.length < 9) ||
      this.user_phone != ""
    ) {
  ...
  methods: {
    ...
    pay_go_click() {
      ...
      if (
        this.store.min_delivery_amt &&
        this.store.min_delivery_amt > this.cost
      ) {
        this.showToast("최소 배달 금액보다 예약 금액이 작습니다.");
...
.delivery-container > .insert_phone,
.delivery-container > .pay_select {
  margin-top: 0.5rem;
  background-color: white;
  padding: 1rem;
...
.delivery-container > .btn_order,
.delivery-container > .btn_pay {
  height: 4rem;
}
.pay_check_postpay {
  position: relative;
