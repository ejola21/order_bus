...
<div class="packing-container">
        ...
        <div v-if="store.order_yn == 'Y' && store.pay_yn == 'Y'" class="pay_check_postpay">
            <div @click="postpay_click">
             <v-radio-group v-model="select_radio" hide-details>
                ...
                <span :class="[{'postpay_desc' : !user_agent}, {'mix_padding' : user_agent}]">&nbsp;&nbsp;(카운터에서 직접 결제)</span>
                </v-radio-group>
                <v-divider class="hr_tag" shadowInset/>
            </div>
        </div>
    ...
    <div class="insert_phone" :class="{'not_only_order' : store.pay_yn == 'Y'}">
      <div class="phone_number" style="background-color: #eee; padding: 0.5rem;">
        <v-badge v-if="store.pay_yn == 'Y' && user_agent_chk" content="2" circle color="#0954ba" badgeClass="badge_size">
            <span class="phone_title">&nbsp;휴대폰 번호 입력</span>
        </v-badge>
        <v-badge v-else-if="store.pay_yn == 'Y' && !user_agent_chk" content="2" circle color="#0954ba" badgeClass="badge_size_other">
            <span class="phone_title">&nbsp;휴대폰 번호 입력</span>
        </v-badge>
        <v-badge v-else-if="store.pay_yn != 'Y' && user_agent_chk" content="1" circle color="#0954ba" badgeClass="badge_size">
            <span class="phone_title">&nbsp;휴대폰 번호 입력</span>
        </v-badge>
        <v-badge v-else content="1" circle color="#0954ba" badgeClass="badge_size_other">
            <span class="phone_title">&nbsp;휴대폰 번호 입력</span>
    ...
    <div class="arrive_time">
      <div class="store_time" style="background-color: #eee; padding: 0.5rem;">  
        ...
        <v-badge v-else-if="store.pay_yn != 'Y' && user_agent_chk" content="2" circle color="#0954ba" badgeClass="badge_size">
            <span class="phone_title">&nbsp;매장 도착 시간</span>
        </v-badge>
        ...
        <div class="time_select">
            <v-select  v-model="time" label="선택해주세요" solo  
                 :items="items"  item-text="title" item-value="value" >
            </v-select>
        </div>
    ...
    <div v-show="select_radio == '선불'" class="order_btns">
      ...
      <v-col cols="12" sm="6">
        <pay-btn v-show="select_radio == '선불'" @pay_go_click="pay_go_click" :btn_name="'결제하기 (포장)'"/>
              
        <v-btn x-large color="primary" dark class="btn_order2" v-show="select_radio == '후불'"  @click="order_click">주문하기 (포장)</v-btn>
      </v-col>
...
.packing-container>.btn_order, .packing-container>.btn_pay {
    height: 4rem;
}
