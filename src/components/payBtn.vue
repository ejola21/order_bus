  ...
  <div class="payBtn">
    ...
    <form ref="kg_kakao" method="post" :action="api_url + 'kg_call_web'">
      ...
      <input
        ...
        :value="api_url + 'kg_approval'"
      />
      <input type="hidden" name="allat_enc_data" />

      <input
        type="hidden"
        name="allat_settle_agency_amt"
        v-model="agency_amt"
      />
    ...
    <form
      ...
      ref="settle_app"
      method="post"
      action="https://pg.settlebank.co.kr/card/MbNewCardAction.do"
    >
      <input
        type="hidden"
        name="PMid"
        :value="store.sb_auth_id"
      /><!--세트뱅크 가맹점 상점ID -->
  ...
  methods: {
    pay_send(params) {
      ...
      if (params.pay_gb != "NORMAL") {
        if (params.pay_gb == "APPCARD") this.card_gb = params.card_gb;
      } else {
        let total_price = 0;
