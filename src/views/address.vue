  ...
  <div class="address_container">
          ...
          <v-row v-show="!address_select">
            <v-col cols="8" class="ml-3">
              <v-text-field label="동 또는 도로명 입력하세요" prepend-inner-icon="mdi-magnify" v-model="s_addr" solo></v-text-field>
            </v-col>
                
            
            <v-col cols="3">
              <v-btn class="mt-1" large v-on:click="search" primary  @click="search">검색</v-btn>
            </v-col>
        </v-row>
        ...
        <div v-show="address_select" class="select_addr_div">
            <div class="selectItem">
                <img class="select_img" src="../assets/img/search_place.png" width="20rem"/>
                    <div class="jibun">
                    {{this.jibun_address}}
                    </div>
                    <div class="road">
                ...
                <div class="address_detail">
                  <v-text-field placeholder="상세주소 입력" full-width v-model="address_detail"></v-text-field>
                    
                </div>
                <div class="address_detail" v-if="store.delivery_name_yn === 'Y'">
                  <v-text-field placeholder="주문자 성함 입력" full-width v-model="user_name"></v-text-field>
                    
                </div>
                <div class="address_add_div">
                  <v-btn primary block v-if="store.delivery_name_yn === 'Y'" class="address_add_btn"  @click="addr_add" >주소/이름 입력 완료</v-btn>
                  <v-btn primary block v-else class="address_add_btn"  @click="addr_add" >주소 입력 완료</v-btn>
                </div>
        </div>
    </div>
</template>
