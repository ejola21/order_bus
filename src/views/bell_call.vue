...
export default {
  ...
  methods: {
    getData() {
      ...
      this.$store.dispatch("f_user_store_bell", param).then((res) => {
        // console.log("res", res);
        if (res.length > 0) {
    ...
    req_bell_call() {
      ...
      this.$store.dispatch("f_c_store_bell", param).then((res) => {
        this.$store.commit("is_popup", {
          is_popup: false,
          title: "",
          desc: "",
        ...
        if (res.result == "SUCCESS") {
          ...
          this.showToast("이미 호출 하셨습니다. 죄송하지만 잠시만 기다리세요.");
        } else this.showToast("통신에러 입니다. 잠시후에 다시 시도해 주세요.");
      });
    },
