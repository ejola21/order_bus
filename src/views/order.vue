<template>
  <div class="order">
    ...
    <div class="title_name_img">
      <div class="main_image inline_block">
        ...
        <div
          ...
          v-if="
            s_product.sale_gb == 'T' &&
              (timeCalc(s_product.sale_st_time1, s_product.sale_ed_time1) ||
                timeCalc(s_product.sale_st_time2, s_product.sale_ed_time2))
          "
        >
          <div class="grid_pruduct_title">
            <div slot="title" class="packing_sale_desc time_sale_content">
              포장시 추가 할인
            </div>
          </div>
        </div>
    ...
    <div class="product_desc inline_block">
      <div class="product_title_name inline_block">
    ...
    <div class="product_option">
      <div class="select_option_name" v-if="s_product.p_grp_id1 > 0">
        <div class="select_option_title option_margin">
          {{ options1[0].p_grp_name }}
        </div>
        <div
          class="select_option_item option_margin"
    ...
    <div class="product_option">
      <div class="select_option_name" v-if="s_product.pm_grp_id > 0">
        <div class="select_option_title option_margin">
          {{ m_options[0].p_grp_name }}
        </div>

        <div
          class="select_option_item option_margin"
          v-for="(option, index) in m_options"
        >
          ...
          >
            ...
            <v-col cols="1">
              <v-checkbox
                v-model="chk[index]"
                v-on:change="
                  checkChange(option.p_o_val, option.p_o_id, chk[index])
                "
              >
              </v-checkbox>
            </v-col>
    ...
    <div
      v-if="s_product.sold_out_yn == 'N' && s_product.price_yn == 'Y'"
      class="group"
    >
      <div class="item">
        {{
          s_product.sale_price > 0
            ? formatPrice(s_product.sale_price, "default")
            : formatPrice(s_product.price, "default")
        }}원
      </div>
      <div class="icon_span">
        인원
    ...
    <div
      v-if="s_product.sold_out_yn == 'N' && s_product.sale_gb == 'T'"
      class="price"
    >
      <div class="price_title">총 예약 금액</div>
    ...
    <div
      ...
      :style="{
        'background-image': 'url(' + require('../assets/img/noimg.jpg') + ')',
      }"
      style="background-size: 100% 100%"
    >
      <lazy-component>
    ...
    <v-row no-gutters v-else>
      <v-col cols="6">
        <v-btn
          ...
          width="100%"
          x-large
          v-on:click="basketClick()"
          >장바구니 담기</v-btn
        >
      </v-col>
      <v-col cols="6">
        <v-btn
          class="btn_basket_move"
          color="primary"
          width="100%"
          x-large
          v-on:click="payClick()"
      ...
      <!-- <div  class="btn_basket_move" v-on:click="payClick()" value="바로 예약">바로 예약</div> -->
    </v-row>
  </div>
</template>
...
export default {
    ...
    if (this.s_product.p_grp_id2 > 0) {
      ...
      if (idx2 > -1) {
        ...
        this.radio_option_price =
          ...
          parseInt(this.options2[idx2].p_o_val);
      }
    }
  ...
  methods: {
    ...
    checkChange(value, id, status) {
      ...
      if (status) this.pm_o_id.push(id);
      else {
    ...
    calcPrice() {
      this.total_price =
        ...
        parseInt(this.cnt);
      this.select = "예약 " + this.formatPrice(this.total_price) + "원";
    ...
    basketClick() {
      if (this.order_product.length > 9) {
        ...
        if (this.memo.length > 50) {
          this.showToast(" 예약요청사항을 50자리 이내로 입력하세요");
        } else if (this.total_price < 100) {
          this.showToast(" 예약금액은 100원 이상이여야 합니다.");
    ...
    payClick() {
      ...
      if (this.memo.length > 50) {
        ...
        var product = {
          // 예약하기 클릭시 파람값으로 product 키에 this.s_product를 밸류로 담습니다.
          product: this.s_product,
          p_o_id1: this.p_o_id1,
          p_o_id2: this.p_o_id2,
          pm_o_id: this.pm_o_id,
