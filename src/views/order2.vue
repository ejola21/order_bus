    ...
    <div class="order21">
      ...
      <div class="card_input">
        <div class="cd_no_title">
          카드 번호
          <span v-if="cd4.length == 0" class="cd_desc_title"
            >* 1회 결제시 카드가 저장됩니다.</span
          >
        </div>

        <div class="cd_no">
          <input
            class="cd1 input_card_box"
            v-model="cd_no1"
            type="tel"
            @keyup="cd_no1_onlyNum"
            maxlength="4"
          />
          <input
            class="cd_pw input_card_box"
            v-model="cd_no2"
            type="tel"
            @keyup="cd_no2_onlyNum"
            maxlength="4"
          />
          <input
            class="cd_pw input_card_box"
            v-model="cd_no3"
            type="tel"
            @keyup="cd_no3_onlyNum"
            maxlength="4"
          />
          <input
            class="cd_last input_card_box"
            v-model="cd_no4"
            type="tel"
            @keyup="cd_no4_onlyNum"
            maxlength="4"
          />
        </div>
        ...
        <div class="cd_exp">
          <input
            class="exp_mon input_card_box"
            v-model="PExpMon"
            type="tel"
            @keyup="exp_mon_onlyNum"
            maxlength="2"
          />
          <span>&nbsp;/&nbsp;</span>
          <input
            class="exp_year input_card_box"
            v-model="PExpYear"
            type="tel"
            @keyup="exp_year_onlyNum"
            maxlength="2"
          />
        </div>
      </div>
...
export default {
      ...
      container2.onkeyup = function(e) {
        ...
        if (myLength >= maxLength) {
          if (target.getAttribute("class"))
            var aa = target.getAttribute("class");
          var next = target;
  ...
  methods: {
    ...
    pay_send() {
      ...
      if (
        this.calcObj.total_price > 1000000 &&
        this.store.store_id != "dg500004"
      ) {
        //대구 책나무만 아니면 100만원 이하만 결제 되도록
        this.showToast("1,000,000원 초과시 예약 되지 않습니다.");
        return;
      } else if (
        this.calcObj.total_price > 3000000 &&
        this.store.store_id == "dg500004"
      ) {
        //대구 책나무만 300만원 이하만 결제 되도록
        this.showToast("3,000,000원 초과시 예약 되지 않습니다.");
        return;
      }
      ...
      this.$store.dispatch("c_order", param).then((res) => {
        ...
        this.$router.push(
          "/pay_result/" + res.PStateCd + "/" + res.PRmesg1 + "/SB"
        );
