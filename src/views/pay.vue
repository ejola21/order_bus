  ...
  <div v-if="l_pay.length == 0" class="no_content">
      <div class="error-mark-wrap">
        <div class="empty_pay">
          ...
          <div class="con">주문 후, 결제내역을 확인해보세요 ^^</div>
        </div>
      </div>
    </div>
  <!-- <mu-paper class="no_content"  v-if ="l_pay.length == 0 " >
    <div class="error-mark-wrap"><img src="../assets/img/error_outline.svg" slot="left" class="error-mark"/><strong class="point_color2">주문현황</strong>이 없습니다.</div>
  </mu-paper> -->
  ...
  <v-card class="pay_container" max-width="500">
    <v-list-item v-for="(pay, index) in l_pay" :key="index">
      <v-list subheader class="payment_date">
       ...
       <v-subheader  v-if="index > 0 && pay.dt != l_pay[index-1].dt">{{pay.dt}}</v-subheader>
